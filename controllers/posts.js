...
exports.selectOnePost = (req, res) => {
    ...
    mysqlConnection.query("SELECT * FROM Posts WHERE id_posts = ?", [id_posts], (err, rows, fields) => {
        if (!err) {
            res.send(rows[0]);
        } else {
            console.log(err);
...
exports.insertOnePost = (req, res) => {
    ...
    mysqlConnection.query(sql, [posts.img, posts.title, posts.description], (err, rows, fields) => {
        if (!err) {
            res.send(rows);
        } else {
            console.log(err);
...
exports.deleteOnePost = (req, res) => {
    let id_users = req.params.id_users;
    let id_posts = req.params.id_posts;
    mysqlConnection.query("DELETE FROM Posts WHERE id_posts = ?", [id_posts], (err, rows, fields) => {
        if (!err) {
